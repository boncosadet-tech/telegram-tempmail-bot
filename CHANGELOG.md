...
- **Rewrite README** for professional polish: bilingual tagline, badges, TOC,
  role-based "Untuk siapa" table, ASCII architecture diagram, clear 5-minute
  Quick Start, consolidated step-by-step setup, power-user CLI reference table,
  env bindings + module tables, Cloudflare quota matrix, FAQ, and security
  summary. Both newcomers and power users can find what they need faster.
- Update repository URLs in `package.json` to point at the active mirror
  `moahaassy-design/telegram-tempmail-bot` so npm links land on the correct
  GitHub page.
