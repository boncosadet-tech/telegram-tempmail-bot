...
void main() {
  test('normalizeDomain strips scheme, path, and casing', () {
    expect(InputValidators.normalizeDomain('HTTPS://Excalibur.Email/app'),
        'excalibur.email');
  });
  ...
  test('normalizeScriptName matches worker naming constraints', () {
    expect(InputValidators.normalizeScriptName('', 'Example.COM'),
        'telegram-tempmail-example-com');
    expect(InputValidators.normalizeScriptName('My Worker!!', 'example.com'),
        'my-worker');
  });

  test('Telegram token validator detects bot token shape', () {
    expect(
        InputValidators.isTelegramBotToken(
            '123456789:ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghi_123'),
        isTrue);
